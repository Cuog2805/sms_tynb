...
namespace VnptSmsBrandName.Service
{
	public class MSmsService: BaseService, IMSmsService
	{
		...
		public async Task<ServiceResult<MSmsViewModel>> SendMessage(MSmsViewModel model, List<IFormFile> fileDinhKem, List<long> selectedFileIds, Users user)
		{
			// validate input
			if (user == null)
				return ServiceResult<MSmsViewModel>.Failure("L?i khi x? lý SendMessage: L?i khi thông tin ngu?i dùng!");

			if (string.IsNullOrEmpty(model.Content))
				return ServiceResult<MSmsViewModel>.Failure("L?i khi x? lý SendMessage: Không có n?i dung tin nh?n!");

			if (model.Employees.Count == 0)
				return ServiceResult<MSmsViewModel>.Failure("L?i khi x? lý SendMessage: Chua ch?n ngu?i nh?n tin nh?n!");
			...
			{
				// C?p nh?t l?i n?u wpSms dã du?c t?o
				if (wpSms != null)
				{
					try
					{
		...
		private async Task<string> HandleFileAttachments(List<IFormFile> fileDinhKem, List<long> selectedFileIds, Users user, long smsId, string domain)
		{
			...
			// X? lý files dã ch?n t? selectedFileIds
